...
class UserManager {
	...
	public static void dataSetting(String userData) {
		...
		for (int i = 0; i < data.length; i++) {
			...
			int code = Integer.parseInt(user[0]);
			String pw = user[1];
			String name = user[2];
			User temp = new User(code, name, pw);
			list.add(temp);
		}
	}
	...
	public int returnCode(int idx) {
		return list.get(idx).getCode();
	}
	...
	public void createUser(String name, String pw) {
		User user = new User(rCode(), name, pw);
		list.add(user);
	...
	public void updateUser(int code, String update, int size) {
		int idx = findUseridx(code);
...
class Account {
	...
	public Account(int code, String account, int money) {
		this.code = code;
		this.account = account;
		this.money = money;
	}
...
class AccountManager {
	...
	public static void dataSetting(String accountData) {
		...
		for (int i = 0; i < data.length; i++) {
			String[] accData = data[i].split("/");

			int code = Integer.parseInt(accData[0]);
			String acc = accData[1];
			int money = Integer.parseInt(accData[2]);

			Account account = new Account(code, acc, money);
			list.add(account);
		}
	}
...
class Bank {
	...
	private void loadData() {
		for (int i = 0; i < userManager.returnSize(); i++) {
			int code = userManager.returnCode(i);
			
				int size = accountManager.loadCount(code);
				System.out.println(size);
				userManager.updateUser(code, "LOAD", size);
	...
	private boolean pwCheck(int code) {
		String pw = inputString("비밀번호 입력");
		int idx = userManager.findUseridx(code);

		if (userManager.checkPw(idx, pw))
			return true;
		else
			return false;
	}
